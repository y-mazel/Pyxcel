...
def _addMethod(fldName, clsName, verb, methodMaker, dict_):
    ...
    dict_[methodName] = methodMaker(compiledName)


def _getCompiledName(fldName, clsName):
    """Return mangled fldName if necessary, else no change."""
    # If fldName starts with 2 underscores and does *not* end with 2
    # underscores...
...
def _makeSetterReal(compiledName):
    """Return a method that sets compiledName's value."""
    return lambda self, value: setattr(self, compiledName[:-4], value + 1.0J *
                                       self.__getattribute__(compiledName[:-4])
                                       .imag)
...
class Meta(type):
    ...
    def __new__(cls, clsName, bases, dict_):
        for fldName in dict_['__dict__'].keys():
            if np.iscomplex(dict_['__dict__'][fldName]):
                _addMethod(fldName + 'Real', clsName, 'get', _makeGetterReal,
                           dict_)
                _addMethod(fldName + 'Real', clsName, 'set', _makeSetterReal,
                           dict_)
                _addMethod(fldName + 'Imag', clsName, 'get', _makeGetterImag,
                           dict_)
                _addMethod(fldName + 'Imag', clsName, 'set', _makeSetterImag,
                           dict_)
                _addMethod(fldName, clsName, 'get', _makeGetter, dict_)
                _addMethod(fldName, clsName, 'set', _makeSetter, dict_)
...
def MakeClasses(InstrumentParameters={'Wavelength': 1.54, 'Coordinates': 1},
                ...
                SampleParameters={'Stacks': [], 'Ambient': None,
                                  'Substrate': None},
    ...
    class Instrument:
        ...
        def __repr__(self):
            ...
            for k in list(self.__dict__.keys()):
                ...
                if isinstance(self.__getattribute__(k), str):
                    stemp = "%s = '%s'," % (k, uni(self.__getattribute__(k)))
                else:
                    stemp = '%s = %s,' % (k, uni(self.__getattribute__(k)))
                s = s + stemp
            return s[:-1]+')'
        ...
        def load(self, file_):
            ...
            if ModelID == dbase['ModelID']:
                ...
                print('ERROR: File ModelID doesnt match the one for the ' +
                      'Model used')
                print('File ModelID: ', dbase['ModelID'], ', Loaded ModelID: ',
                      ModelID)
            dbase.close()
    ...
    class Stack:
        ...
        def resolveLayerParameter(self, parameter):
            par = ([lay.__getattribute__(parameter) for lay in self.Layers] *
                   self.Repetitions)
            return par
    ...
    class Sample:
        ...
        def _fromdict(self, dic):
            ...
            for key in dic.keys():
                if key in self.__dict__:
                    if key not in ('Stacks', 'Ambient', 'Substrate'):
                        self.__setattr__(key, dic[key])
                else:
                    error = True
            if error:
        ...
        def SimSpecular(self, TwoThetaQz, instrument):
            return SimulationFunctions['Specular'](TwoThetaQz, self,
                                                   instrument)

        def SimOffSpecular(self, TwoThetaQz, ThetaQx, instrument):
            return SimulationFunctions['OffSpecular'](TwoThetaQz, ThetaQx,
                                                      self, instrument)

        def SimSLD(self, z, instrument):
            if 'SLD' in SimulationFunctions:
                return SimulationFunctions['SLD'](z, self, instrument)
            else:
                return {}

        def SimGIXRF(self, element, XRFline, TwoThetaQz, det_angle_array,
                     instrument, inc_flux, sol_ang_det_array, det_efficiency):
            return SimulationFunctions['GIXRF'](element, XRFline, TwoThetaQz,
                                                det_angle_array, self,
                                                instrument, inc_flux,
                                                sol_ang_det_array,
                                                det_efficiency)
